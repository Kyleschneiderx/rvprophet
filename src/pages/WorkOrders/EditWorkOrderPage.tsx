...
export const EditWorkOrderPage = () => {
  ...
  useEffect(() => {
    if (workOrder && !initialized) {
      ...
      setLaborHours(workOrder.laborHours);
      setLaborRateOverride(workOrder.laborRate);
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: (payload: { status: WorkOrderStatus }) =>
      api.updateWorkOrder(workOrderId!, {
        issueDescription,
        photos,
        parts,
        laborHours,
    ...
    onSuccess: (_data, variables) => {
      ...
      setTimeout(() => navigate(`/work-orders/${workOrderId}`), 1500);
    },
    onError: (error) => {
      ...
      <div className="space-y-6">
        <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-6">
          ...
          <Link
            ...
            className="mt-4 inline-block rounded-xl bg-yellow-600 px-4 py-2 text-sm font-semibold text-white"
          >
            View Work Order
          </Link>
        </div>
      ...
      <section className="space-y-4">
        ...
        <Card>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <button
              type="button"
              disabled={isPending || !issueDescription}
